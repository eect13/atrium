...
- Finance: Markets first (official PSEi 30, Yahoo PSEi index last, free-float weighting, Yahoo screener with PE/cap filters, sparks, research PDF with an Expert take) and Books (cash, bank, wallet, card). Open a ticker for Facts vs Rumors. Add any name from the plus button. Stock tape in Options: Auto and Yahoo both keep PSE last on this desk — Yahoo dropped .PS listings
...
- Notes: Windows Sticky Notes baseline — colored pads, title + body, new/delete/color, list + board, pin to float — plus format (B/I/U/S/bullets), pictures, pencil with undo. Float, color, and delete sit on the title bar and only appear when the pointer is near (always on a phone). Color opens a clamped grid. Delete asks first. Windows menu Notes Open floats every pad, Close unpins them — same as Weather
...
Needs Microsoft JDK 17, Android SDK (`%LOCALAPPDATA%\Android\Sdk`), NDK, Rust `aarch64-linux-android`. Output: `deploy/android/atrium-v1.2.14-arm64-release.apk`.
