...
function copyApksToDeploy(sdk, env, preferSigned) {
  ...
  const friendly = join(OUT, "atrium-arm64-release.apk");
  if (primary && existsSync(primary)) {
    copyFileSync(primary, friendly);
    console.log(`  ${friendly}`);
    try {
      const ver = readPackageVersion();
      const versioned = join(OUT, `atrium-v${ver}-arm64-release.apk`);
...
function patchAndroidSystemBars() {
  const marker = "atrium-system-bars-v4";
  ...
  const body = `package app.atrium.desktop
...
class MainActivity : TauriActivity() {
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    // ${marker}
    enableAtriumWebViewZoom()
    applyAtriumSystemBarInsets()
  }

  private fun applyAtriumSystemBarInsets() {
  ...
  // atrium-webview-zoom
  private fun enableAtriumWebViewZoom() {
    try {
...
`
  ...
  for (const file of files) {
    ...
    let out = body;
    if (pkgMatch && pkgMatch[1] !== "app.atrium.desktop") {
      out = out.replace("package app.atrium.desktop", `package ${pkgMatch[1]}`);
    }
    if (existing.includes(marker) && existing.includes("applyAtriumSystemBarInsets")) {
...
function enableAndroidWebViewZoom() {
  ...
  const marker = "atrium-webview-zoom";
  const roots = [join(appSrc, "java"), join(appSrc, "kotlin")].filter((d) => existsSync(d));
  /** @type {string[]} */
  const files = [];
  function walk(dir) {
...
`;
  for (const file of files) {
    ...
    if (!src.includes("enableAtriumWebViewZoom()")) {
...
console.log("Atrium — Android APK (Tauri, not a PWA)\n");
...
if (preferCargo) {
  ...
  console.log("\nSideload deploy\\android\\atrium-arm64-release.apk (IndexedDB stays on device).");
  process.exit(0);
}
...
if (tauriStatus === 0) {
  ...
  copyFileSync(primary, join(OUT, "atrium-arm64-release.apk"));
  for (const apk of apks) {
    const dest = join(OUT, basename(apk));
    copyFileSync(apk, dest);
    console.log(`  ${dest}`);
  }
  console.log(`  ${join(OUT, "atrium-arm64-release.apk")}`);
